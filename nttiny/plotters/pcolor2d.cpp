...
namespace nttiny {
  ...
  auto Pcolor2d<T>::draw(ImPlotRect& shared_axes, UISettings& ui_settings) -> bool {
      ...
      if (ImPlot::BeginLegendPopup(Sim->get_field_names()[this->m_field_selected])) {
        ...
        ImGui::BeginGroup();
        {
...

          {
            ...
            ImGui::PushID("vmax");
            ImGui::DragFloat(
              "##", &vmax, 0.01f * std::fabs(vmax - vmin), vmin, +FLT_MAX, "%.3e");
            ImGui::PopID();
            ...
            ImGui::PushID("vmin");
            ImGui::DragFloat(
              "##", &vmin, 0.01f * std::fabs(vmax - vmin), -FLT_MAX, vmax, "%.3e");
            ImGui::PopID();
          }
          ImGui::PopItemWidth();
          ...
          /* ---------------------------- colormap selector --------------------------- */
          {
            ...
            ImGui::SameLine();
            if (ImGui::BeginCombo("##",
                                  gp.ColormapData.GetName(gp.Style.Colormap),
    ...
    if (this->m_autoscale && Sim->m_data_changed) {
      this->rescaleMinMax();
    }
    ImPlot::PopColormap();
