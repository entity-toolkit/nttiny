  ...
  void Plot2d<T>::outlineDomain(UISettings& ui_settings) {
    ...
    if (coord == Coord::Spherical) {
      ...
      for (int i { 0 }; i < thres; ++i) {
        ...
        auto  p01     = ImPlot::PlotToPixels(
          ImPlotPoint(rmin * std::sin(phi_min), rmin * std::cos(phi_min)));
        auto p02 = ImPlot::PlotToPixels(
          ImPlotPoint(rmin * std::sin(phi_max), rmin * std::cos(phi_max)));
        ImPlot::GetPlotDrawList()->AddLine(p01, p02, color, thickness);
        auto p11 = ImPlot::PlotToPixels(
          ImPlotPoint(rmax * std::sin(phi_min), rmax * std::cos(phi_min)));
        auto p12 = ImPlot::PlotToPixels(
          ImPlotPoint(rmax * std::sin(phi_max), rmax * std::cos(phi_max)));
