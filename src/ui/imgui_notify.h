...
public:
  ...
  NOTIFY_INLINE auto get_title() const -> const char* { return this->title; };
  ...
  NOTIFY_INLINE auto get_content() const -> const char* { return this->content; };
...
NOTIFY_INLINE void RenderNotifications() {
  ...
  for (std::size_t i = 0; i < notifications.size(); i++) {
    ...
    const auto opacity
        = current_toast->get_fade_percent(); // Get opacity based of the current phase
    ...
    {
      PushTextWrapPos(vp_size.x / 3.f); // We want to support multi-line text, this will wrap the
                                        // text after 1/3 of the screen width
      ...
      // In case ANYTHING was rendered in the top, we want to add a small padding so the text (or
      // icon) looks centered vertically
