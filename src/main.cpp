...
int main() {
...

  {
    ...
    auto myloop = [](GLFWwindow* window, std::vector<unsigned int> programs) {
      ...
      glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), (const void*)0);	// Vertex attributes stay the same
      glEnableVertexAttribArray(0);

      glBindVertexArray(VAOs[1]);	// note that we bind to a different VAO now
      glBindBuffer(GL_ARRAY_BUFFER, VBOs[1]);	// and a different VBO
      glBufferData(GL_ARRAY_BUFFER, sizeof(secondTriangle), secondTriangle, GL_STATIC_DRAW);
      glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, (const void*)0); // because the vertex data is tightly packed we can also specify 0 as the vertex attribute's stride to let OpenGL figure it out
      glEnableVertexAttribArray(0);
